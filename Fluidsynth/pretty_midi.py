...
class PrettyMIDI(object):
    ...
    def fluidsynth(self, fs=44100, sf2_path=None, start_new_process32=False):
        ...
        if start_new_process32:
            from tempfile import mktemp
            from librosa.core import load
            from subprocess import check_output
    
            # Write the midi file in the temp directory
            filename_temp = mktemp('.mid')
            self.write(filename_temp)
            
            # Call a subprocess, starting a new python32 thread
            new_process_module = "C:\Users\Alexis\Source\TestPython2\AutomaticAudioTranscript/fluidsynth_new_process.py"    
            filename_wav = check_output("python {} {} {}".format(new_process_module, filename_temp, fs))
            
            # Reload the .wav file, stored in the temp directory    
            synthesized = load(filename_wav, sr = fs)[0] # !!!CHANGE THE SR
            # DONT FORGET TO CLEAN AFTERWARD!!!
            return(synthesized)                            
